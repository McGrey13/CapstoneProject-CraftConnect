...
Route::middleware([])->group(function () {
    ...
    Route::get('/products/{id}/related', [ProductController::class, 'getRelatedProducts'])->whereNumber('id');
    ...
    Route::post('/orders-test/{orderId}/cancel', function($orderId) {
        try {
            ...
            // Determine if this request is an admin-initiated auto-cancel or force-cancel
            $reason = request()->input('reason', request()->getContent() ? json_decode(request()->getContent(), true)['reason'] ?? '' : '');
            $forceFlag = filter_var(request()->input('force', false), FILTER_VALIDATE_BOOLEAN);

            // Consider it an auto-cancel when reason mentions 'auto-cancel' (case-insensitive) or force flag is true
            $isAutoCancel = false;
            if (!empty($reason) && stripos($reason, 'auto-cancel') !== false) {
                $isAutoCancel = true;
            }
            if ($forceFlag) {
                $isAutoCancel = true;
            ...
            // Check if order can be cancelled under normal rules
            $canCancelNormally = in_array(strtolower($order->status), ['pending', 'processing']) && 
                        (strtolower($order->payment_method ?? 'cod') === 'cod' || $order->paymentStatus === 'pending');

            // Allow cancellation if normal rules permit it OR this request is an admin auto-cancel
            $canCancel = $canCancelNormally || $isAutoCancel;
...
Route::middleware(['auth:sanctum'])->group(function () {
    ...
    Route::prefix('orders')->group(function () {
        ...
        Route::post('/{order}/pay', [PaymentController::class, 'reinitiatePayment']);
    ...
    Route::middleware(['auth:sanctum'])->group(function () {
        ...
        Route::prefix('admin')->group(function () {
            ...
            Route::post('/reviews/{reviewId}/flag', [AdminController::class, 'flagReview']);
            Route::post('/reviews/{reviewId}/unflag', [AdminController::class, 'unflagReview']);
        });
