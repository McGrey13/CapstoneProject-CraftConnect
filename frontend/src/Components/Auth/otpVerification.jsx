...
const OtpVerification = () => {
  ...
  useEffect(() => {
    // If no email is provided and user is already logged in, redirect to home
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data && res.data.message === 'Account verified successfully') {
        // Save authentication data
        localStorage.setItem("auth_token", res.data.token);
        localStorage.setItem("user", JSON.stringify(res.data.user));

        // Handle redirection logic
        const nextDestination = location.state?.redirectTo;
        if (nextDestination) {
          ...
          navigate(redirectMap[res.data.user.role] || "/home");
        } else {
          // Default redirect
          navigate("/home");
        }
