...
class AfterSaleController extends Controller
{
    ...
    public function createRequest(Request $request)
    {
        try {
            ...
            // Notify admins about the return/refund request
            try {
                $customerName = $customer->user ? $customer->user->userName : 'Unknown Customer';
                NotificationService::notifyAdminsReturnRefundRequest(
                    $afterSaleRequest,
                    $order,
                    $customerName
                );
            } catch (\Exception $adminNotifException) {
                // Log notification error but don't fail the request creation
                Log::warning('Failed to send admin notification for return/refund request: ' . $adminNotifException->getMessage(), [
                    'request_id' => $afterSaleRequest->request_id,
                ]);
            }
