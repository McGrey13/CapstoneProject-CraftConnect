...
const Orders = () => {
  ...
  const handlePayNow = async (order) => {
    ...
    try {
      ...
      // Log response for debugging redirect issues
      console.debug('Payment API response:', response?.data);

      if (response.data?.success && response.data?.checkout_url) {
        // If backend simulated the checkout (no PayMongo key), navigate to app success handler
        if (response.data?.simulated) {
          const sourceId = response.data.source_id || '';
          // Navigate to frontend orders page with payment success query so useEffect can pick it up
          window.location.href = `/orders?payment=success&source_id=${encodeURIComponent(sourceId)}`;
          return;
        }

        // For real PayMongo checkout URLs, use assign to navigate (preserves history behavior)
