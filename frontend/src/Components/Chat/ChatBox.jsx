...
export default function ChatBox({ conversationId, user }) {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/chat/${conversationId}/messages`, {
          headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8000/api/chat/${conversationId}/send`,
        formData,
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
              ...
              >
                <div className="text-xs mb-1">
                  <span className={`px-2 py-0.5 rounded ${
                    m.sender_id === user.id ? "bg-blue-600" : "bg-gray-200"
                  }`}>
                    {m.message_type}
                  </span>
                </div>
                
                <p className="break-words">{m.message_text || "📎 Attachment only"}</p>
                
                {m.attachments && m.attachments.map((a, i) => (
                  <div key={i} className="mt-2">
                    {a.file_type === "image" ? (
                      <img 
                        src={`http://localhost:8000/storage/${a.file_url}`} 
                        alt="attachment" 
                      ...
                      <a 
                        href={`http://localhost:8000/storage/${a.file_url}`}
                        target="_blank" 
                        rel="noopener noreferrer"
                        className={`flex items-center gap-2 ${
                          m.sender_id === user.id ? "text-blue-100" : "text-blue-500"
                        } hover:underline`}
                      >
