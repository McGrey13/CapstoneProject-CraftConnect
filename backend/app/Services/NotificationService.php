...
class NotificationService
{
    ...
    /**
     * Notify all admins about a pending product that needs approval.
     */
    public static function notifyAdminsPendingProduct($productName, $sellerName, $productId): void
    {
        ...
        foreach ($admins as $admin) {
            self::create(
                ...
                "Product '{$productName}' from seller '{$sellerName}' is pending approval.",
                "/admin/products",
                [
                    'product_id' => $productId,
                    'product_name' => $productName,
                    'seller_name' => $sellerName,
                    'action_type' => 'pending_product',
                ]
            );
        }
    }
    ...
    /**
     * Notify all admins about a flagged review with offensive language.
     */
    public static function notifyAdminsFlaggedReview($reviewId, $productName, $customerName, $comment): void
    {
        $admins = User::where('role', 'administrator')->get();
        
        // Truncate comment if too long
        $truncatedComment = strlen($comment) > 100 ? substr($comment, 0, 100) . '...' : $comment;
        
        foreach ($admins as $admin) {
            self::create(
                $admin->userID,
                'admin_action',
                'Review Flagged - Offensive Language',
                "Review from '{$customerName}' on product '{$productName}' contains offensive language: \"{$truncatedComment}\"",
                "/admin/reviews",
                [
                    'review_id' => $reviewId,
                    'product_name' => $productName,
    ...
    /**
     * Notify all admins about a new return/refund request.
     */
    public static function notifyAdminsReturnRefundRequest($afterSaleRequest, $order, $customerName): void
    {
        $admins = User::where('role', 'administrator')->get();
        
        $requestType = ucfirst($afterSaleRequest->request_type);
        $message = "Customer '{$customerName}' has submitted a {$requestType} request for order #{$order->order_number}.";
        ...
        foreach ($admins as $admin) {
            self::create(
                ...
                [
                    ...
                    'request_type' => $afterSaleRequest->request_type,
                    'customer_name' => $customerName,
                    'action_type' => 'return_refund_request',
                ]
            );
        }
    }
